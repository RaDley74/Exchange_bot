...
class ReferralHandler:
    ...
    async def _display_referral_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 1):
        ...
        min_payout = self.bot.config.min_referral_payout
        ...
        text = (
            ...
            f"⚔️ Если вы пришли от конкурента, напишите в поддержку и предоставьте доказательства — вам будет зачислено  $5.\n\n"
            f"🔗 **Ваша реферальная ссылка:**\n`{referral_link}`\n\n"
            f"💰 **Ваш реферальный баланс:** ${referral_balance:.2f}\n"
            f"_(Вывод средств возможен при балансе от ${min_payout:.2f})_\n\n"
        )
